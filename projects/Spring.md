...
簡単営業向けてのERMアプリです。正社員が所有するPCや備品や車が更新・保守は必要になれば上司に自動メールを打つアプリになります。
...
#### データベース
データベースはPostgresを使いました。Postgresの訓練しようとおもって使いました。
...
#### Mybatis
生SQLメソッド作られます。プライマリーキーと外部キーでジョインして複数テーブルの情報を取得できます。
...
#### Health service
従業員の持っている物の自動Health確認を行っています。物が更新及び保守は必要と判断判断された場合自動に上司でメール送ります。
この自動物確認スクリプトはＢｏｓｓＤａｓｈｂｏａｒｄで開始もできます。

#### Ｃｒｏｎ　ｊｏｂ
スクリプトを毎日同じときに自動で開始するためＣｒｏｎを使いました。5日ごとにメール履歴を削除します。
